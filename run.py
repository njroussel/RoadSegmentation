...
def main(argv=None):
    ...
    if global_vars.POST_PROCESS:
        ...
        for i in range(1, global_vars.TRAINING_SIZE + 1):
            ...
            p_img = pred_help.get_prediction_image(
                test_data_filename, i, s, model, file_regex, means, stds, global_vars, max_thresh)

            Image.fromarray(p_img).save(test_dir + "prediction_" + str(i) + ".png")

            oimg = pred_help.get_prediction_with_overlay(
                test_data_filename, i, s, model, file_regex, means, stds, global_vars, max_thresh)

            oimg.save(test_dir + "overlay_" + str(i) + ".png")

    if ((not global_vars.POST_PROCESS) and global_vars.TEST_PREDICTIONS):
        ## Run on test set.
        print("\n******************************************************************************")
    ...
    if global_vars.POST_PROCESS:
        ...
        if global_vars_pp.TEST_PREDICTIONS:
            ...
            for i in range(1, TEST_SIZE + 1):
                ...
                p_img = pred_help.get_prediction_image(
                    test_data_filename, i, s, model, file_regex, means, stds, global_vars_pp, 
                    max_thresh)
                
                p_img = img_help.quantize_binary_images(
                    [p_img], global_vars_pp.IMG_PATCH_SIZE, global_vars.IMG_PATCH_SIZE)[0]
